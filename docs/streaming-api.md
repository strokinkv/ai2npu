...
`partial_silence_ms` управляет частичными результатами: `0` выключает `delta`,
любое ненулевое значение должно быть меньше `default_min_silence_ms`. Чем меньше
порог, тем раньше клиент видит partial, но тем чаще Whisper запускается на NPU.
...
- `conversation.item.input_audio_transcription.delta`:
  ...
  партиал — полный прогон Whisper на NPU. Если очередной partial-декод не является
  расширением уже отправленного текста, сервер не шлёт ревизию и ждёт финального
  `...completed`.
...
`...completed`; `delta` и `completed` всегда имеют `content_index: 0`; события
`...completed` упорядочены по фразам (`item_id`), не теряются и не дублируются;
`speech_started`/`speech_stopped` обрамляют каждую фразу. Одна VAD-фраза = один
Realtime-«item».
...
Обычный `cargo test` запускает детерминированные тесты протокола и VAD. Живой
NPU smoke-тест включается отдельно переменными окружения:
...
Тест ожидает 16-bit PCM WAV и принимает любое количество `delta`, потому что их
число зависит от аудио, VAD-пауз и `partial_silence_ms`.
