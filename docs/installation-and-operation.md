...
В дефолтном конфиге Whisper (`openai/whisper-large-v3-turbo`) имеет
`preload = true` и `queue_timeout_sec = 0`: модель прогревается при старте службы,
удерживается в памяти и запросы ждут очереди без таймаута. BGE по умолчанию
загружается лениво. Для освобождения ресурсов используйте `ai2npu.exe unload`.
...
При старте сервис читает `config.toml`, инициализирует файловое логирование, определяет устройства OpenVINO и запускает локальный HTTP-сервер. Модели с `preload = true` загружаются сразу; остальные загружаются лениво по первому запросу. Инференс проходит через общую FIFO-очередь, поэтому одновременно выполняется только один запрос к модели. Для `queue_timeout_sec = 0` ожидание старта inference в очереди не ограничено таймером.
